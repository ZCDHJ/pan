...
class Content
{
   ...
   <a href="http://service.weibo.com/share/share.php?url=' . $archive->permalink . '&title=' . $archive->title . '" target="_blank" itemprop="breadcrumb" title="" data-toggle="tooltip" data-original-title="' . _mt("分享到微博") . '" onclick="window.open(this.href, \'weibo-share\', \'width=550,height=335\');return false;"><i style ="font-size:15px;" class="fontello fontello-weibo" aria-hidden="true"></i></a>' . $screenshot . '</div>';
    ...
    public static function exportPostPageHeader($archive, $isLogin, $isCustom = false)
    {
        ...
        if (@$expert["post_speech"] !== false){
            $html .= '<a class="speech-button m-l-sm superscript" data-toggle="tooltip" data-original-title="' . _mt("朗读文章") . '"><i data-feather="play-circle"></i></a>';
        }
...
EOF;
    ...
    public static function exportPayForAuthors()
    {
        ...
        $returnHtml = '
             <div class="support-author">
                 <button id="support_author" data-toggle="modal" data-target="#myModal" class="btn btn-pay btn-danger btn-rounded"><i class="fontello fontello-wallet" aria-hidden="true"></i>&nbsp;' . _mt("赞赏") . '</button>
    ...
    public static function exportPostFooter($time, $obj, $archive)
    {
        ...
        if (@$expert["show_post_last_date"] !== false) {
    ...
    public static function returnHeaderImgSrc($widget, $select, $index = 0, $flag = false)
    {
        ...
        switch ($thumbSelect) {
            case -1:
                $imgSrc = Content::whenSwitchHeaderImgSrc($widget, $index, $options->RandomPicChoice,
                    $widget->attachments(1)->attachment, $widget->content, $widget->fields->thumb);
                break;
            case 0://全部关闭
                break;
            case 1://全部开启
    ...
    public static function getPostParseWay()
    {
        ...
        if (!Device::isIE()) {
            if (Utils::isPluginAvailable("Handsome_Plugin", "Handsome")) {
                $way = Helper::options()->plugin('Handsome')->parseWay;
            }
        }
        return $way;
...
EOF;
     ...
     */
        ...
        if (@$attrs['id'] !== null) {//云解析
            $getUrl = THEME_URL . 'libs/interface/Get.php?id=' . @$attrs["id"] . '&type=' . @$attrs["type"] . '&media=' .
                @$attrs["media"];
        } else {
            $getUrl = "";
        }
...
eof;
    ...
    public static function videoParseCallback($matches)
    {
        ...
        $playCode = '<div class="kyt-player"><video src="' . $url . '" style="background-image:url(' . $pic . ');background-size: cover;"></video></div>';
            ...
            if (count($posts) == 0) {
                ...
                if ($cover == "" || $cover == "http://") {
                    ...
                    $thumbArray = $db->fetchAll($db
                        ...
                        ->orWhere('cid = ?', $cid)
                        ->where('name = ? ', 'thumb'));
                    $targetImgSrc = Content:: whenSwitchHeaderImgSrc(null, 0, 2, null, $result['text'],
                        @$thumbArray[0]['str_value']);
...
EOF;
    ...
    public static function parseButtonCallback($matches)
    {
        ...
        if (@$attrs['icon'] != "") {//判断是否是feather 图标
            if (count(mb_split(" ", $attrs['icon'])) > 1) {
...
EOF;
    ...
    public static function returnLeftItem($asideItem, $haveSub, $subListHtml)
    {
        ...
        if (trim($itemFeather) == "") {
            if (count(mb_split(" ", $itemClass)) == 1 && strpos($itemClass,"-") === false) {
...
<a class="right carousel-control" href="#index-carousel" role="button" data-slide="next">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="glyphicon-chevron-right" aria-hidden="true"><polyline points="9 18 15 12 9 6"></polyline></svg>
...
//                    print_r("\nmid".$obj->mid."|count:".$obj->count."\n");
                    $name[] = $obj->name;
                    if ($obj->count > $max_single) {
                        $max_single = $obj->count;
                    }
                    $num [] = $obj->count;
                }else{
                    $num [$i] += $obj->count;
