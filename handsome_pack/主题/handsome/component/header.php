...
<head>
    ...
    <?php
    $expert = json_decode($this->options->expert, true);
    if ($expert == null) {
        $expert = array();
    }
    ?>
    ...
    <script type="text/javascript">
        ...
        window['LocalConst'] = {
            ...
            MATHJAX_CDN: '<?php echo $PUBLIC_CDN_ARRAY["js"]["mathjax"] ?>',
            ...
            PAGE_ANIMATE: '<?php in_array('isPageAnimate', $this->options->featuresetup) ?>',
            ...
            THEME_MATHJAX: '<?php echo in_array('mathJax', $this->options->featuresetup); ?>',
            THEME_TOC: '<?php echo IS_TOC; ?>',
            THEME_DARK_MODE: '<?php echo(@$this->options->dark_setting == "" ? "light" : $this->options->dark_setting)
                ?>',
            THUMB_STYLE: '<?php echo $this->options->thumbArrangeStyle; ?>',
            AUTO_READ_MODE: '<?php echo @in_array('autoReadMode',$this->options->featuresetup) ? true : false ?>',

            //代码高亮
            CODE_STYLE_LIGHT: '<?php $this->options->codeStyle() ?>',
            CODE_STYLE_DARK: '<?php $this->options->dark_codeStyle() ?>',
            ...
            SHOW_IMAGE_ALT: '<?php echo !@array_key_exists("alt_show", $expert) ? true : @$expert["alt_show"] ?>',
            USER_LOGIN: '<?php echo $this->user->hasLogin(); ?>',
            USE_CACHE: '<?php $cache = "no"; if (Utils::isPluginAvailable("Handsome_Plugin", "Handsome")) {
                $cache = Helper::options()->plugin("Handsome")->cacheSetting;
            } echo $cache === "yes";?>',
            POST_SPEECH: '<?php echo !@array_key_exists("post_speech", $expert) ? true : @$expert["post_speech"] ?>',
        };
    ...
    <?php else: ?>
    <?php Utils::getFileList("origin", "css") ?>
