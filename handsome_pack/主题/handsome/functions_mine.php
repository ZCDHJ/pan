...
//    约定，定义所有的url末尾都要有斜杆
    ...
    if (strlen(trim($options->LocalResourceSrc)) > 0){//主题静态资源的绝对地址
        ...
        @define('STATIC_PATH',str_ireplace(BLOG_URL,trim($cdnArray[0])."/",THEME_URL)."assets/");
    } else{
    ...
    if (Utils::isPluginAvailable("Handsome_Plugin","Handsome")){
        $sticky = Helper::options()->plugin('Handsome')->sticky_cids;
...
且该功能为实验功能，如有bug，请及时反馈");
    ...
    $themetypeEdit = new Text("themetypeEdit",NULL,"white-white-white","主题色调自定义搭配","<b style='color: red'>如果你不知道这是什么，也不知道怎么填，请清空该项</b></br> 该项比上一个设置项「主题色调选择」优先级更高，你可以自定义搭配主题已有的颜色 <a href='https://handsome.ihewro.com/#/color'>使用文档</a>");
...
<code>rgba(0,0,0,0.3)</code>，填写后你就会发现效果的，就是让透明色不太过透明，导致字都看不清了。</br></br> 一般来说，".redText('根据背景图片的主色调来填比较好看。')." </br>举例：背景图片主色调是蓝色，那就填 <code>rgba(56, 136, 255, 0.3)</code> (56,136,255) 就是蓝色，0.3 代表背景的透明度。"));
    ...
    $dark_setting = new Radio("dark_setting",
        ...
        ),'auto',"夜间/日间模式","自动：会先根据浏览者的操作系统的主题色设置 </br> 如果用户操作系统没有该设置，则根据用户浏览器的时间进行设置（早6点~晚18点为浅色模式）");
    ...
    $dark_codeStyle = new Select('dark_codeStyle',
        array(
            ...
            'vs' => 'vs',
            'xcode' => 'xcode',
        ),'solarized-dark','深色模型下的代码高亮风格选择','使用的代码高亮风格见 <a href="https://highlightjs.org/">风格列表</a>');
    ...
    $codeStyle = new Select('codeStyle',
        array(
            ...
            'vs' => 'vs',
            'xcode' => 'xcode',
        ),'solarized-light','浅色模型下代码高亮风格选择','使用的代码高亮风格见 <a href="https://highlightjs.org/">风格列表</a>');
...
协议头。</br>如果为空，则默认是 <code>xxx.com/cross.html</code>默认的地址为时光机的地址，请看  <a href="https://handsome.ihewro.com/#/page">使用文档——独立页面（时光机）</a> ');
    ...
    //轮播图
    $wheel = new Textarea('wheel',NUll,NULL,_mt("首页轮播图设置"),"配置首页轮播图，<b style='color: red'>如果不需要，请清空该项</b><a href='https://handsome.ihewro.com/#/functions?id=%e9%a6%96%e9%a1%b5%e8%bd%ae%e6%92%ad%e5%9b%be%e8%ae%be%e7%bd%ae' target='_blank'>使用文档</a>");
    $form->addInput($wheel);
...
{"name":"网易云音乐","img":"https://s2.ax1x.com/2019/07/20/ZzU3LD.png","value":"你的网易云账号","link":"#"}', _mt('时光机联系方式配置').' <a href="https://handsome.ihewro.com/#/customize" target="_blank"><span style="">'._mt('配置文档').'</span></a>', _mt('基本配置直接修改框中的value值和link值即可，高级配置请看文档'));
    ...
    //时光机配置码
    $time_code = new Text('time_code',NULL,"default","时光机身份验证编码","用于微信公众号发送时光机验证个人身份的编码，请勿告诉任何人。如果编码泄露，别人可以通过该编码在微信公众号向的博客添加说说。你可以随时更新此编码，以便不被别人使用。<a href='https://handsome.ihewro.com/#/wechat'>图文教程</a>");
...
每两项之间用英文逗号隔开。最后一项请勿添加逗号。<a href="https://handsome.ihewro.com/#/functions">使用文档</a> ');
    ...
    //语言设置
    $language = new Select('language', I18n_Options::listLangs(), 'zh_CN','界面语言', '默认固定为简体中文, 「自动」即根据浏览器的语言设置自动选择语言。');
...
类型</code> </br> 举例： <code>https://assets.ihewro.com | UPYUN</code> 在服务商的镜像空间配置加速域名为 <code>https://www.ihewro.com</code> </br>   支持以下服务商：</br>七牛云「QINIU」</br>又拍云「UPYUN」</br>阿里云「ALIOSS」</br> 腾讯云「QCLOUD」具体细节请看 <a href="https://handsome.ihewro.com/#/speed">详细使用文档</a></br><b style="color:red;">如果在Handsome插件中开启了前端使用Vditor.js 解析，则文章的图片暂时无法替换</b>'));
...
我们使用的图片大小尺寸很多时候是大于所需要的尺寸（div的尺寸），造成图片加载缓慢，启动第一项配置会自动使用云存储服务商提供的图片处理对图片进行处理，以便加载更小的体积。具体细节请看 <a href=\"https://handsome.ihewro.com/#/functions\">详细使用文档</a>");
    ...
    $publicCDNSelcet = new Select('publicCDNSelcet',array(
        ...
        '5' => 'catCDN',
        '6' => '本地'
    ),'4',_t('选择公共CDN库'),_t('主题除了使用本地一些静态资源，还引用了bootstrap/jquery 库的公共cdn地址（体积较大>60kb），通常使用托管在公共cdn的资源会获得更快的访问速度</br>默认选择BootCDN，因为各大公共CDN库的目录结构有所差异，暂不支持自定义CDN库。
<br/><span style="color: red">注意：bootcdn在有些地区可能无法正常访问，此时你可以更换其他的公共CDN，比如百度或七牛，稳定性比较好。使用 <code>本地</code>表示直接加载你的服务器上面资源</span>'));
    ...
    $adContentSidebar = new Textarea('adContentSidebar', NULL,NULL,_t('全局右侧边栏广告位'),_t('此处可以填写HTML代码，广告位的位置在右侧的边栏中。</br>关于投放google广告可以看这位博主的使用教程，我自己没有测试过<a href="https://www.imydl.tech/status/571.html">Typecho 下 Preference 主题投放谷歌AdSense广告总结</a>'));
    ...
    $expertSetting = new Textarea('expert', NULL,NULL,_t('开发者高级设置'),_t('此处填写高级设置，具体请参见<a target="_blank" href="http://handsome.ihewro.com/#expert">使用文档</a>'));
...
function getFriendWall()
{
    ...
    if ($expert == null) $expert = array();
    $num = 51;
    if (array_key_exists("rank_num",$expert)){
        $num = @$expert["rank_num"];
    }
    $db = Typecho_Db::get();
...
EOF;
    ...
    $mostactive = "<h4 class='font-bold m-b-lg m-t-lg'>"._mt("总评论排行榜")."</h4>";
    $echoCount = 0;
    if (count($result) > 0) {
