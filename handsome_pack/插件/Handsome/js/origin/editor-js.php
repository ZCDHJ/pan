...
function choseOrigin($content)
{
    ...
    <script>
        var hplayerUrl = '<?php echo $options->themeUrl?>/libs/interface/Get.php';
    ...
    <script src="<?php echo $options->pluginUrl ?>/Handsome/js/editor.min.js"></script>
    <link rel="stylesheet" href="<?php echo $options->themeUrl ?>/assets/css/origin/owo.min.css" type="text/css"/>
    <link rel="stylesheet" href="<?php echo $options->pluginUrl ?>/Handsome/css/editor.css" type="text/css"/>
    <?php
...
function choseVditor($content)
{
    ...
    $cssUrl = $options->pluginUrl . '/Handsome/css/index.min.css?v=7.3.0';
    $jsUrl = $options->pluginUrl . '/Handsome/js/index.min.js?v=7.3.0';
    ...
    <script>
        ...
        $(document).ready(function () {
             ...
             * @param type input类型：input 输入框,select 下拉框
             * @param options
            ...
            options.strings = {
                ...
                musicdialog: {
                    "type": "music",
                    "intro": '<p><b>插入音乐</b></p><p>请在下方的输入框内输入要插入的音乐地址 </p><p style="color: #ff0012">支持云解析歌曲地址和本地歌曲资源，不支持歌单地址</p><a target="_blank" href="https://handsome.ihewro.com/#/functions?id=%e6%96%87%e7%ab%a0%e5%86%85%e6%8f%92%e5%85%a5%e9%9f%b3%e4%b9%90">使用文档</a>',
                    "input": [
                        returnDialogInputItem("", "MUSIC_URL", "http://", "", [], true, "input", "." +
                            "./usr/themes/handsome/libs/interface/Get.php",
                            "MUSIC_ID"),
                    ],
                ...
                citeDialog: {
                    ...
                    "ret": '[scode type="CITE_TYPE"]这里编辑标签内容[/scode]'
                },
                buttonDialog: {
                    type: "button",
                    intro: "<p><b>插入按钮</b></p>",
                ...
                postDialog: {
                    ...
                    "ret": '[post cid="POST_CID" cover="POST_THUMB"/]'
                },
                videoDialog: {
                    "type": "video",
                    ...
                    "input": [
                        returnDialogInputItem("输入视频地址（必填,如xxx.mp4）", "VIDEO_URL", "", ""),
                        returnDialogInputItem("输入视频封面（可不填）", "VIDEO_THUMB", "http://", ""),
                    ],
                    "ret": '[vplayer url="VIDEO_URL" pic="VIDEO_THUMB" /]'
                },
                collapseDialog: {
                    type: "collapse",
                    "intro": '',
                ...
                function construct() {
                    ...
                    dialogContent.input.forEach(function (element) {
                        ...
                        if (element.type === "input") {
                            const _input = w.createElement("input");
                            _input.type = "text";
                            _input.id = element.id;
                            if (element.value === "") {
                                _input.value = element.prefix;
                            } else {
                ...
                function getRet() {
                    ...
                    if (typeof ajax_url != "undefined" && ajax_url !== "") {
                        const callback = $.ajax({
                            ...
                            url: ajax_url,
                            data: {
                                data: $('.wmd-prompt-dialog input').val(),
                                size: "large",
            ...
            const custom_toolbar = [
                ...
                'preview',
                {
                    ...
                    click: function () {
                        window.open("https://auth.ihewro.com/user/docs/#/vditor")
                    }
                },
                'edit-mode',
                {
